...
class BasicDataLoader(object):
    ...
    def __init__(self, config, word2id, relation2id, entity2id, tokenize_name, data_type="train"):
        logger.info("start init {} DataLoader...", data_type)
    ...
    def _load_file(self, config, data_type="train"):
        ...
        self.fact_num = index
        self.batches = np.arange(self.fact_num)
    ...
    def _load_data(self):
        ...
        self.query_texts = np.full((self.fact_num, self.max_query_word), self.num_word, dtype=int)  # 将问题使用 tokenizer 进行编码
        self.query_entities = np.zeros((self.fact_num, self.max_local_entity), dtype=float)  # 每个问题的主题词，用 1 标识子图编号
        self.candidate_entities = np.full((self.fact_num, self.max_local_entity), len(self.entity2id), dtype=int)  # 每个问题的候选实体，对应 id 使用全局 id
        self.seed_distribution = np.zeros((self.fact_num, self.max_local_entity), dtype=float)  # 每个问题的主题词概率，如果有 k 个，赋值为 1/k
        self.kb_adj_mats = np.empty(self.fact_num, dtype=object)  # 局部子图关系列表(head_list, rel_list, tail_list)
        self.answer_dists = np.zeros((self.fact_num, self.max_local_entity), dtype=float)  # 每个问题的答案，用 1 标识子图编号
        self.answer_lists = np.empty(self.fact_num, dtype=object)  # 每个问题的答案列表，对应 id 使用全局 id
        self.__prepare_data()
    ...
    def __build_global2local_entity_maps(self):
        ...
        """
        global2local_entity_maps = [None] * self.fact_num
    ...
    def _build_fact_mat(self, sample_ids, fact_dropout):
        ...
        for i, sample_id in enumerate(sample_ids):
            ...
            batch_heads = np.append(batch_heads, head_list[mask_index] + index_bias)  # 保证同一个 batch 内的局部编码不一致
            batch_rels = np.append(batch_rels, rel_list[mask_index])
            batch_tails = np.append(batch_tails, tail_list[mask_index] + index_bias)
    ...
    def reset_batches(self, is_sequential=True):
        if is_sequential:
            ...
            self.batches = np.random.permutation(self.fact_num)
...
class SingleDataLoader(BasicDataLoader):
    ...
    def get_batch(self, iteration, batch_size, fact_dropout, test=False):
        ...
        end = min(batch_size * (iteration + 1), self.fact_num)
